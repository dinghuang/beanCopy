...
import static org.dinghuuang.bean.util.Collections.asSet;

import org.dinghuuang.bean.function.Extractor;
...
public class Strings {
    ...
    /**
     * Returns a variable name which doesn't conflict with the given variable names existing in the
     * same scope and the Java keywords.
     *
     ...
     * @return a variable name based on the given original name, not conflicting with any of the
     * given other names or any Java keyword; starting with a lower-case letter
     */
     ...
     * @return the identifier without any characters that are not allowed as part of a Java
     * identifier.
     */
    public static String sanitizeIdentifierName(String identifier) {
        if (identifier != null && identifier.length() > 0) {
            ...
            if (firstAlphabeticIndex < identifier.length()) {
                ...
                String firstAlphaString = identifier.substring(firstAlphabeticIndex)
                    .replace("[]", "Array");
    ...
    /**
     * Returns a stub property name from full class name by stripping away the package and
     * decapitalizing the name For example will return {@code fooBar} for {@code
     * com.foo.bar.baz.FooBar} class name
     *
     ...
     * <p>
     * E.q. for the name {@code props.font} it will return an {@link Iterable} containing the {@code
     * props} and {@code font}
     *
